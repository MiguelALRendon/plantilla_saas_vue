...
- Almacenar persistent key property name en entity class prototype usando PERSISTENT_KEY_KEY Symbol efficient O(1) metadata lookup durante URL formatting HTTP request preparation
...
### PERSISTENT_KEY_KEY Symbol

Identificador único usado property key prototype almacenar persistent key property name string. Implementación: `export const PERSISTENT_KEY_KEY = Symbol('persistent_key')`. Storage: `Product.prototype[PERSISTENT_KEY_KEY] = 'slug'`. **NOTA:** El nombre contiene "_KEY" duplicado intencional por convención del framework. Symbol provides collision-free key metadata storage evitando conflicts real properties methods entity protecting namespace integrity. Prototype-level storage (all instances share) porque persistent key property es type-level configuration NO instance-specific data: entire entity class usa same property identifier uniformly all persistence operations.
...
public static getPersistentKey(): string {
    const persistentKey = this.prototype[PERSISTENT_KEY_KEY];
